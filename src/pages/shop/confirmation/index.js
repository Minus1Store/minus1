...
const CheckoutPage = ({location}) => {
    ...
    return (
        <PageLayout>
            <SEO titleTemplate={'%s | Cart'} url={location.href} description={`Select your pay and shipping preferences in Minus1 shop checkout.`}/>
                            ...
                            <table className={styles.products}>
                                <tbody>
                                    {confirmationData.cart.map((cartItem, index) => {
                                        return <CartProduct presentational={true} key={index} data={cartItem}/>
                                    })}
                                </tbody>
                            </table>
                        </div>
