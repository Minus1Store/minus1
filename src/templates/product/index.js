...
const ProductPage = ({ data, location }) => {
  ...
  const [selectedSize, setSelectedSize] = useState(
    data.product.data.sizes &&
      data.product.data.sizes[0].size.document &&
      data.product.data.sizes[0].size.document.data.title
  )
  ...
  const [nextLink, setNextLink] = useState(
    nextProductLink(
      data.product.uid,
      data.allSubcategoryProducts.edges,
      data.allCategoryProducts.edges
    )
  )
  ...
  useEffect(() => {
    if (localStorage.getItem('cart')) {
      let cartItems = [...cart]
      let thisProducts = cartItems.filter(
        (item) => item.uid == data.product.uid
      )
  ...
  function nextProductLink(productUID, primaryProducts, secondaryProducts) {
    if (primaryProducts.length > 0) {
      let productIndex = primaryProducts.findIndex(
        (product) => product.node.uid == productUID
      )
      ...
      let productIndex = secondaryProducts.findIndex(
        (product) => product.node.uid == productUID
      )
  ...
  const checkIfThereAreSizes = () => {
    return data.product.data.sizes.every(
      (node) =>
        node.size.document &&
  ...
  return (
    <PageLayout minimizedHeader>
      <SEO
        ...
        description={`Product ${
          data.product && data.product.data.title
        }. Color ${data.product && data.product.data.color_name}. Price €${
          data.product && data.product.data.price
        }. About product: ${
          data.product &&
          data.product.data.description &&
          data.product.data.description.text
        }. Sizes: ${
          data.product &&
          data.product.data.sizes &&
          data.product.data.sizes.length > 0 &&
          data.product.data.sizes.map(({ size }) => size).join(',')
        }`}
      />
      <div className={styles.pageWrapper}>
        ...
        <div className={styles.mainImageContainer}>
          {data.product &&
            ...
            data.product.data.images[mainImageNum] && (
              <ReactImageMagnify
                smallImage={{
                  alt: data.product.data.images[mainImageNum].image.alt,
                  isFluidWidth: true,
                  src:
                    data.product.data.images[mainImageNum].image.localFile.url,
                }}
                largeImage={{
                  width: 800,
                  height: 800,
          ...
          {data.product && data.product.data.description && (
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: data.product.data.description.html,
              }}
            ></div>
          )}
          ...
          <div className={styles.row}>
            ...
            {nextLink && (
              <Link to={nextLink} className={styles.link}>
                next{' '}
                {(data.product &&
                  data.product.data.product_subcategory &&
                  data.product.data.product_subcategory.document &&
                  data.product.data.product_subcategory.document.data
                    .product_subcategory) ||
                  (data.product &&
                    data.product.data.product_category &&
                    data.product.data.product_category.document &&
                    data.product.data.product_category.document.data
                      .product_category)}{' '}
                &gt;
              </Link>
            )}
            ...
            {checkIfThereAreSizes() &&
              ((data.product &&
                ...
                <div onClick={() => !alreadySelected && addProductToCart()}>
                  <PrimaryButton
                    text={alreadySelected ? `in basket` : `add to basket`}
...
export const pageQuery = graphql`
  query ProductBySlug(
    ...
    allCategoryProducts: allPrismicProduct(
    ...
    allSubcategoryProducts: allPrismicProduct(
      filter: {
    ...
    allFamilyProducts: allPrismicProduct(
      filter: { data: { product_family: { uid: { eq: $family_uid } } } }
    ) {
      edges {
        node {
          uid
