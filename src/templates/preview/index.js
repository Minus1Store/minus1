  ...
  return (
    <PageLayout>
      <SEO
        ...
        description={`${
          data.currentPreview && data.currentPreview.data.title
        } Preview. Our preview products: ${
          data.previewFamilies &&
          data.previewFamilies.edges.length > 0 &&
          data.previewFamilies.edges
            .map(({ node }, index) => node.data.family_name)
            .join(',')
        }`}
      />
            ...
            data.previewFamilies.edges.map(({ node }, index) => {
              return (
                <li key={index} className={styles.thumbnail}>
                  <Link
                    to={`/previews/${
                      node.data.preview && node.data.preview.uid
                    }/${
                      node.data.product_category &&
                      node.data.product_category.uid
                    }/${node.uid}`}
                  >
                    {node.data.family_main_image &&
                      node.data.family_main_image.localFile &&
                      node.data.family_main_image.localFile.childImageSharp &&
                      node.data.family_main_image.localFile.childImageSharp
                        .fluid && (
                        <Image
                          fluid={
                          ...
                          alt={node.data.family_main_image.alt}
                        />
                      )}
                  </Link>
                </li>
